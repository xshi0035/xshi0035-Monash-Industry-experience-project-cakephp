...
<div class="d-sm-flex align-items-center justify-content-between flex-wrap">
    <h2>Deactivated Staff Accounts</h2>
    <?php if ($this->request->getSession()->read('Auth.role_id') == 1): ?>
        <div class="ml-auto">
            <?= $this->Html->link(__('Return to List of Active Staff'), ['action' => 'index'], ['class' => 'btn btn-secondary mr-2 mb-2 mb-sm-0']) ?>
        </div>
    <?php endif; ?>
...
<div class="card shadow mb-4">
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered" id="archivedUsersTable" width="100%" cellspacing="0">
                <thead>
                    <tr>
                        <!-- <th></th> -->
                        <th>Name</th>
                ...
                <tbody>
                    <?php foreach ($users as $user): ?>
                        <tr>
                            ...
                            <?php if ($this->request->getSession()->read('Auth.role_id') != 2): ?>
                                <td class="actions">
                                    <?= $this->Html->link(__('Reactivate'), ['action' => 'reactivate', $user->id]) ?>
                                </td>
                            <?php endif; ?>
                        </tr>
                    <?php endforeach; ?>
...
<script>
    $(document).ready(function() {
        ...
        var table = $('#archivedUsersTable').DataTable({
            lengthChange: false,
            columnDefs: [
                {
                    targets: 1,
